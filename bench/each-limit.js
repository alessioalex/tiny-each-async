...
suite('asynchronous .eachLimit() modules', function() {
  ...
  bench('eachAsync()', function(cb) {
    eachAsync(items, 2, function(item, next) {
      setImmediate(next);
    }, cb);
  });

  bench('async.each()', function(cb) {
    async.eachLimit(items, 2, function(item, next) {
      setImmediate(next);
    }, cb);
  });
});
